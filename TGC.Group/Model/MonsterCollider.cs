...
//SE COMENTO LA GRAVEDAD PORQUE DE MOMENTO NO SE NECESITA

namespace TGC.Collision
{
	/// <summary>
	...
	public class MonsterCollider
	{
			...
			List<TgcBoundingAxisAlignBox> obstaculos, int recursionDepth)
		{
			...
			if (TgcCollisionUtils.intersectRayPlane(slideRay, slidePlane, out slideT, out slideDestinationPoint))
			{
				...
				var slipperyslideMovementVector = Vector3.Normalize(slideMovementVector) * movementVector.Length();
				...
				doCollideWithWorld(characterSphere, slipperyslideMovementVector, obstaculos, recursionDepth + 1);
			}
		}
